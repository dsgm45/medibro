...
echo "=== MediBro: Registration rate limiting, logout hardening, real client IP fix ==="
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .patient-sidebar a,
        .patient-sidebar button.sidebar-logout {
            display: flex;
            align-items: center;
            gap: 10px;
            padding: 10px 12px;
        ...
        .patient-sidebar button.sidebar-logout:hover {
    ...
    <nav>
        ...
        <div style="display: flex; align-items: center; gap: 16px;">
            {% if session.get('user_id') %}
                ...
                <form action="{{ url_for('logout') }}" method="POST" style="margin: 0; display: inline;">
                    <input type="hidden" name="csrf_token" value="{{ csrf_token() }}">
                    <button type="submit" style="color: #dc2626; text-decoration: none; font-weight: 600; background-color: #fee2e2; padding: 6px 12px; border-radius: 6px; border: none; font-family: inherit; font-size: inherit; cursor: pointer;">Log Out</button>
                </form>
            {% else %}
...
FILEEOF_2
cat > templates/appointment_summary.html << 'FILEEOF_3'
{% extends "base.html" %}
...
{% endblock %}
FILEEOF_6
cat > templates/medicines.html << 'FILEEOF_7'
{% extends "base.html" %}
...
{% endblock %}
FILEEOF_7
cat > templates/my_health.html << 'FILEEOF_8'
{% extends "base.html" %}
...
{% endblock %}
FILEEOF_8
cat > templates/patient_dashboard.html << 'FILEEOF_9'
{% extends "base.html" %}
...
{% endblock %}
FILEEOF_9
cat > templates/sos.html << 'FILEEOF_10'
{% extends "base.html" %}
...
{% endblock %}
FILEEOF_11
cat > templates/vitals.html << 'FILEEOF_12'
{% extends "base.html" %}
...
git add -A
git commit -m "Add registration rate limiting, fix real client IP behind Render's proxy, convert logout to CSRF-protected POST"
git push origin main

echo ""
echo "=== Done. Check Render dashboard for the new deploy. ==="
echo "This also adds ProxyFix middleware so the app correctly sees each"
echo "visitor's real IP address instead of Render's internal proxy address -"
echo "important since the new rate limiter depends on it being accurate."
